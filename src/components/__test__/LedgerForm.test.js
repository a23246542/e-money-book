...
describe('test LedgerForm component', () => {
  ...
  describe('test LedgerForm with no data', () => {
    ...
    it('submit form with invalid price should show alert message', () => {
      ...
      act(() => {
        ...
        wrapper
          .find('#inputDate')
          .simulate('change', { target: { value: '2020-12-14' } });
      });
      wrapper.update();
      wrapper.find('#submit').simulate('click');
      expect(wrapper.find('.alert').length).toEqual(1);
    ...
    it('submit form with invalid date should show alert message', () => {
      act(() => {
        ...
        wrapper
          .find('#inputDate')
          .simulate('change', { target: { value: '2021-12-14' } });
      });
      wrapper.update();
      wrapper.find('#submit').simulate('click');
      expect(wrapper.find('.alert').length).toEqual(1);
    ...
    it('submit form with valid data should trigger callback with right object', () => {
      act(() => {
        wrapper
          .find('#inputTitle')
          .simulate('change', { target: { value: '吃飯' } });
        wrapper
          .find('#inputAmount')
          .simulate('change', { target: { value: '300' } });
        wrapper
          .find('#inputDate')
          .simulate('change', { target: { value: '2020-12-13' } });
      });
  ...
  describe('test LedgerForm with item data', () => {
    ...
    it.only('submit with change value should trigger callback with right object', () => {
      ...
      wrapper2
        .find('#inputTitle')
        .simulate('change', { target: { value: '工資' } });
      wrapper2
        .find('#inputAmount')
